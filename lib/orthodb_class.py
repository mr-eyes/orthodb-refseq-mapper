...
class OrthoDB:
    ...
    _op_thorough = 0
    debug = 0
    __report = {"unfound_geneSymbols": set(), "summary":[]}
    ...
    def odb_genes_info(self, path, tax_id, key, value):
        """
        File: odb*genes.tab
        info = [odb_gene_id, tax_id, prot_seq_id, gene_symbol, ncbi_id, description]
        return : dict {key : [ value(s) ]}
        """
        info_location = {
            "odb_gene_id": 0,
            "tax_id": 1,
        ...
        if key not in info_location.keys() or value not in info_location.keys():
            exit("Please select from [odb_gene_id, tax_id, prot_seq_id, gene_symbol, ncbi_id, description]")
                ...
                if key_idx == 5:
                    ...
                    if _gene_name not in self.gene_name_to_ncbigid:
                        ...
                        if self._op_thorough and not found:
                            ...
                            if not found:                            
                                self.__report["unfound_geneSymbols"].add(_gene_name)
                                continue
                        ...
                        # report to the missings. Skip the current iteration
                            ...
                            self.__report["unfound_geneSymbols"].add(_gene_name) # reporting the unfound geneSymbol
                            continue
